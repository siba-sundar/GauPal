...
export default function CowBreedIdentifier() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Build API URL from environment variable. Do NOT force HTTPS here —
      // forcing https makes the browser attempt a TLS handshake to a plain HTTP
      // backend (this causes the server to log "Invalid HTTP request received.").
      // Build API URL from environment variable. If not set, call the current origin.
      // This avoids malformed URLs like `http:///predict/` when VITE_IDENTIFY_BREED is empty.
      const envBase = (import.meta.env.VITE_IDENTIFY_BREED || "").replace(/\/+$/g, "");
      const originBase = window.location.origin; // includes protocol + host
      const base = envBase || originBase;
      const apiUrl = `${base.replace(/\/+$/g, "")}/predict/`;

      // Let the browser/axios set the Content-Type (and boundary) for multipart/form-data
  console.debug('Sending image to', apiUrl, 'file:', file.name, file.type, file.size);
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
          ...
          {results && results.length > 0 && (
            <div className="space-y-4">
              <div className="bg-green-50 p-4 rounded-md border border-green-100">
                ...
                <div
                  className={`mt-2 inline-block px-2 py-1 rounded-md text-white text-sm ${getBadgeColor(
      ...
      <div className="mt-6 text-sm text-gray-500 text-center">
        This tool identifies cow breeds from 41 different categories with
